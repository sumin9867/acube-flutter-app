...
class _AntiTheftScreenState extends State<AntiTheftScreen> {
  ...
  /// ✅ Check if SOS numbers already exist in SharedPreferences
  Future<void> _checkSosSetup() async {
    final prefs = await SharedPreferences.getInstance();
    final savedNumbers = prefs.getString('sos-${widget.device.id}');

    if (savedNumbers != null && savedNumbers.isNotEmpty) {
      setState(() {
        isSosSetupComplete = true;
        currentStep = 1; // skip SOS setup step
      });
    }
  }
  ...
  void initState() {
    ...
    _checkSosSetup(); // ✅ Run SOS check on screen load
  }
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  // ✅ SOS Number Input Step (Only if not already saved)
                  ...
                  // ✅ Activate/Deactivate Step (if numbers already saved)
                  ...
                  // ✅ Next Button for SOS setup step
                  if (!isSosSetupComplete && currentStep == 0)
                    SizedBox(
                      width: double.infinity,
                      height: 48,
