...
class AnilocationTaskImpl extends IAnilocationTask with AnilocationTaskInitializer {
  ...
  void _rippleStatusListener(AnimationStatus status) async {
    ...
    if (DateTime.now().difference(rippleTimeCount) > description.rippleIdleAfter) {
      return;
    }
  ...
  void _locationListener() {
    if (description.latLngListener != null) {
      ...
      if (needAdjustment) {
        ...
        var easedProgress = Curves.easeInOut.transform(wrapper.locationCtrller.value);
        var currentBearing = (startBearing + shortestRotation * easedProgress + 360) % 360;
        var smoothValue = value.copyWith(
            bearing: currentBearing,
            latitude: wrapper.locationTween.evaluate(wrapper.locationCtrller).latitude,
            longitude: wrapper.locationTween.evaluate(wrapper.locationCtrller).longitude);
